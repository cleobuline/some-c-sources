...
typedef struct {
    OpCode opcode;
    long int operand; // Utilisé pour les indices dans strings ou pour CALL
} Instruction;
...
void executeInstruction(Instruction instr, Stack *stack, long int *ip, CompiledWord *word) {
    ...
    switch (instr.opcode) {
        case OP_PUSH:
            mpz_set_str(result, word->strings[instr.operand], 10); // Charger depuis la chaîne
            push(stack, result);
            break;
        case OP_ADD:
            pop(stack, a);
        ...
        case OP_DOT:
            ...
            break;
        case OP_FLUSH:
            stack->top = -1; // Pas besoin de réinitialiser, déjà initialisé dans initStack
            break;
        case OP_EQ:
            pop(stack, a);
            pop(stack, b);
...
void compileToken(char *token, char **input_rest) {
    ...
    if (strcmp(token, "+") == 0) instr.opcode = OP_ADD;
        ...
        *input_rest = end + 1;
        ...
        mpz_t test_num;
        mpz_init(test_num);
        if (mpz_set_str(test_num, token, 10) == 0) { // Si c’est un nombre valide
