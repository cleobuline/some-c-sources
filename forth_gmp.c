...
typedef enum {
    ...
    OP_PICK, OP_ROLL, OP_PLUSSTORE, OP_DEPTH, OP_TOP,OP_NIP, OP_MOD ,OP_SEE,OP_ALLOT,OP_CREATE,OP_EMIT // Inclut MOD
} OpCode;
...
void addCompiledWord(char *name, Instruction *code, long int code_length, char **strings, long int string_count) {
    ...
    if (existing_index >= 0) {
        // Remplace une définition existante
        ...
        dict_count++;
        // Mise à jour automatique de DP
        int dp_idx = findMemoryIndex("DP");
        if (dp_idx >= 0) {
            mpz_set_si(memory[dp_idx].values[0], dict_count);
        }
    } else {
        ...
        *input_rest = end + 1;
        ...
        if (index >= 0) {
            ...
            if (mpz_set_str(test_num, token, 10) == 0) {
                ...
                *compile_error = 1; // Marque l’erreur ici
            }
            mpz_clear(test_num);
        }
    }
...
void interpret(char *input, Stack *stack) {
    error_flag = 0;           // Indicateur global d’erreur d’exécution
    int compile_error = 0;    // Indicateur local d’erreur de compilation
    char *saveptr;            // Pour strtok_r
    char *token = strtok_r(input, " \t\n", &saveptr);

    while (token && !error_flag) {
        if (compiling) {
            // Mode compilation : on construit un mot
            if (strcmp(token, ";") == 0) {
                // Fin de la définition du mot
                Instruction end = {OP_END, 0};
                currentWord.code[currentWord.code_length++] = end;

                if (compile_error) {
                    // Erreur détectée : on oublie le mot avec FORGET
                    ...
                    if (index >= 0) {
                        ...
                        printf("Definition aborted due to error, %s forgotten\n", currentWord.name);
                    }
                } else {
                    // Pas d’erreur : on finalise le mot dans le dictionnaire
                    if (current_word_index >= 0 && current_word_index < dict_count) {
                        memcpy(dictionary[current_word_index].code, currentWord.code, 
                               currentWord.code_length * sizeof(Instruction));
                        dictionary[current_word_index].code_length = currentWord.code_length;
                        for (int i = 0; i < currentWord.string_count; i++) {
                            if (dictionary[current_word_index].strings[i]) 
                                free(dictionary[current_word_index].strings[i]);
                ...
                compile_error = 0; // Réinitialisation pour la prochaine définition
            } else {
                // Compilation d’un token dans le mot courant
                compileToken(token, &saveptr,&compile_error);
                // L’erreur est gérée dans compileToken, pas ici
            }
        } else {
            // Mode interactif : exécution immédiate
            ...
            if (mpz_set_str(big_value, token, 10) == 0) {
                ...
                if (token) {
                    ...
                    printf("Colon requires a word name\n");
                }
            } else if (strcmp(token, "LOAD") == 0) {
                char *start = saveptr;
                while (*start && (*start == ' ' || *start == '\t')) start++;
...
char dp_cmd[] = "VARIABLE DP "; // Ajoute DROP
	interpret(dp_cmd, &stack);
    int dp_idx = findMemoryIndex("DP");
    if (dp_idx >= 0) {
mpz_set_si(memory[dp_idx].values[0], dict_count); // DP = dict_count (1)
    } else {
