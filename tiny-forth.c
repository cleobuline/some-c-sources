...
typedef enum {
    ...
    OP_END,
    OP_DOT_QUOTE,    // Pour ."
    OP_CR,           // Retour à la ligne
    OP_DOT_S,        // Affichage de la pile
    OP_CASE,
    OP_OF,
    OP_ENDOF,
    OP_ENDCASE
...
// ### Pile de contrôle pour la compilation
...
// ### Pile de boucle pour l'exécution
typedef struct {
    int index;
    int limit;
    int addr;
...
// ### Dictionnaire des mots compilés
CompiledWord dictionary[DICT_SIZE];
int dict_count = 0;

// ## Fonctions sur la pile de données
...
void executeInstruction(Instruction instr, Stack *stack, int *ip, CompiledWord *word) {
    switch (instr.opcode) {
        ...
        case OP_I: {
            if (loop_stack_top >= 0) {
                push(stack, loop_stack[loop_stack_top].index);
            } else {
                printf("I used outside of a loop!\n");
            }
            break;
        }
        case OP_DO: {
        ...
        case OP_BRANCH_FALSE: {
            if (!pop(stack)) *ip = instr.operand - 1;
            break;
        }
        case OP_BRANCH:
            *ip = instr.operand - 1;
        ...
        case OP_DOT_QUOTE: {
            if (instr.operand >= 0 && instr.operand < word->string_count) {
                ...
                printf("Invalid string index for .\": %d\n", instr.operand);
            }
            break;
        }
        case OP_CR:
    ...
    while (token) {
        if (compiling) {
            if (strcmp(token, ";") == 0) {
                ...
                addCompiledWord(currentWord.name, currentWord.code, currentWord.code_length, currentWord.strings, currentWord.string_count);
            ...
            if (strcmp(token, ":") == 0) {
                ...
                int value = strtol(token, &endptr, 10);
                if (*endptr == '\0') {
                    temp.code[0] = (Instruction){OP_PUSH, value};
